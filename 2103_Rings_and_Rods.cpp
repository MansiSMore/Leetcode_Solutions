...
public:
    ...
    {
        unordered_map<int, string> uobj;
        
        for(int i = 1; i < rings.size(); i += 2)
        {
            uobj[rings[i] - '0'] += rings[i - 1];
        }
        
        int cnt = 0;
        for(auto x : uobj)
        {
            if((x.second.find('B') != std::string::npos) && (x.second.find('G') != std::string::npos) && (x.second.find('R') != std::string::npos))
                cnt++;
        }
